...
const EVENTS = {
    '07': [{ name: 'Aung Ko Htet', type: 'disembark' }, { name: 'Mg Mg Lwin', type: 'disembark' }, { name: 'Htoo Htoo', type: 'disembark' }],
    '23': [{ name: 'Ye Phyo Win', type: 'embark' }, { name: 'Kyaw Zin', type: 'embark' }],
};
    ...
    return (
        <div className="w-full flex flex-col gap-6">
            ...
            <div className="flex items-center justify-between">
                <h1 className="text-xl font-medium text-text tracking-wide">Calendar</h1>
            ...
            <div className="bg-surface border border-border rounded-[6px] shadow-card overflow-hidden">
                ...
                <div className="p-4 border-b border-border flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <span className="text-lg font-semibold text-text">{currentMonth}</span>
                        <button className="p-2 hover:bg-gray-100 rounded-md transition-colors">
                            <ChevronRight className="w-5 h-5 text-text" />
                        </button>
                    </div>
                    <button className="text-sm text-brand-blue hover:underline">Today</button>
                </div>

                {/* Days Header */}
                <div className="grid grid-cols-7 border-b border-border bg-brand-light/64">
                    {DAYS.map((day) => (
                        <div key={day} className="py-3 text-center text-sm font-medium text-[#202020] tracking-wide border-r border-border last:border-r-0">
                            {day}
                        </div>
                    ))}
                </div>
                    ...
                    {DATES.map((week, weekIdx) =>
                        week.map((date, dayIdx) => {
                            ...
                            return (
                                ...
                                >
                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm text-text ${date === '23' ? 'bg-brand-blue text-white' : ''
                                        }`}>
                                        {date}
                                    </div>
                                    <div className="w-full mt-2 flex flex-col gap-1">
                                        {events.map((event, idx) => (
                                            <div
                                                key={idx}
                                                className={`w-full py-1 px-2 rounded-[12px] text-center text-[10px] font-medium truncate ${event.type === 'disembark'
